...
| | |
|---|---|
| `ikigai-update` | pull the personal flake, update its inputs, rebuild and switch, restart the shell. `--no-pull` rebuilds from the inputs as locked, `--boot` builds for the next boot and leaves the running system alone (GPU driver changes), `--check` shows what would change, `--available` only says whether anything is newer |

...
ships through nixpkgs and updates with the flake, but must be selected in Steam.
gamemode and gamescope stay per game; LAVD stays Auto. See `docs/system.md` for checks.
...
| Out | zoxide, localsend, chromium, VS Code, flameshot. Proton-GE was excluded on Arch; the NixOS Steam module now ships it. |
